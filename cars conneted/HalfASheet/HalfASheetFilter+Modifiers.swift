//
//  HalfASheetFilter+Modifiers.swift
//  cars conneted
//
...
extension HalfASheetFilter {
    ...
    public func height(_ height: HalfASheetHeight1) -> Self {
        var copy = self
        copy.height = height
        return copy
    }
...
extension View {
    ...
    public func halfASheetFilter<T: View>(isPresented: Binding<Bool>, title: String? = nil, @ViewBuilder content: @escaping () -> T) -> some View {
        modifier(HalfASheetPresentationModifier(content: { HalfASheetPost(isPresented: isPresented, title: title, content: content) }))
    }
}


struct HalfASheetFilterPresentationModifier<SheetContent>: ViewModifier where SheetContent: View {
    
    var content: () -> HalfASheetPost<SheetContent>
    
    init(@ViewBuilder content: @escaping () -> HalfASheetPost<SheetContent>) {
